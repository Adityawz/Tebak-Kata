...
Builder.load_file('TebakKataUI.kv')
Builder.load_file('TitleBar.kv')
Builder.load_file('TabBar.kv')
Builder.load_file('ScoresHistory.kv')

LabelBase.register(name='zekton',
                   fn_regular='font/zekton.ttf')



class tebak_kata(BoxLayout, Screen):
    lb_msg: Label
    ...
    warning_sound = SoundLoader.load('sounds/warning.wav')
    sound_track = SoundLoader.load('sounds/music.wav')
    ...
    def checkAnswer(self):
        ...
        if "".join(self.mutatedWord) in self.words:
            ...
            if self.pb_score.value >= 500:
                self.lb_msg.text = "Jawabanmu Benar!!"
                self.save_score()
            else:
            ...
            self.lb_msg.text = "Masih salah, coba lagi ya!"
    ...
    def showAnswer(self):
        ...
        if self.pb_score.value >= 25:
            ...
            popup = Popup(title='Oops, Peringatan!', content=Label(text="Poin kamu masih\nbelum cukup nih.\nKamu butuh 25 poin\nuntuk mengintip jawaban!"))
...
sm.add_widget(tebak_kata(name='game'))
